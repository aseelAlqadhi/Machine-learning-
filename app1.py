...
def load_models():
    ...
    c_pre = joblib.load("cluster_preprocessor.pkl")
    c_mod = joblib.load("best_clustering_model.pkl")
    return clf, reg, c_pre, c_mod

df = load_data()
best_clf, best_reg, cluster_pre, cluster_model = load_models()
...
# ═══════════════════════════════════════════════════════
    ...
    if st.button("🔮 Predict Now", type="primary", use_container_width=True):
        ...
        # ── Regression input (no cost_of_the_product, no reached_on_time) ──
